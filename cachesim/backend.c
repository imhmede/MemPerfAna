	...
	int Cache__load(Cache* self, addr_range range) {
		...
		for(long cl_id=Cache__get_cacheline_id(self, range.addr); cl_id<=last_cl_id; cl_id++) {
			...
			if(location != -1) {
				...
				foundNode = searchNode(cl_id);
				foundNode->hit++;*/
				//printf("%lld::%ld causes a HIT\n", range.addr, cl_id);
...
#define INITERROR return
		...
		void printmiss()
		{
			struct Node* current = head;
			FILE *fp;
			fp = fopen("miss.out", "w");  // open the file in write mode
			if (fp == NULL) {
				printf("Error opening file.\n");
			}
			while (current != NULL) {
				fprintf(fp,"%ld %ld\n", current->cl_id, current->miss);
				//fprintf(fp, "%d\t%s", 10, text);  // write the text to the file
				current = current->next;
			}
		...
		void printfreq()
		{
			struct Node* current = head;
			FILE *fp;
			fp = fopen("freq.out", "w");  // open the file in write mode
			if (fp == NULL) {
				printf("Error opening file.\n");
			}
			while (current != NULL) {
				fprintf(fp,"%ld %ld\n", current->cl_id, (current->hit + current->miss));
				//fprintf(fp, "%d\t%s", 10, text);  // write the text to the file
				current = current->next;
			}
			fclose(fp);  // close the file
